class User {
    ...
    incProgress(lvl) {
        ...
        lvl = ranks.indexOf(lvl)
        if(lvl === -1) throw new Error('Wrong task rank')
        ...
        let r = ranks.indexOf(this.rank)
        if(lvl - r > 0){
            let d = lvl - r
            this.progress += d * d * 10
        }
        else if(lvl - r === 0) this.progress += 3
        else if(lvl - r > -3) this.progress += 1

        if (this.progress >= 100) {
            let lvlUp = Math.floor(this.progress/100)
            this.progress -= lvlUp * 100
            r += lvlUp
            if(r >= 0 && r < 15){
                this.rank = ranks[r]
            }else if(r >= 15) {
